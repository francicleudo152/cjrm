...
*/
const contaCaracter = possui => possui.length;
const palavra = contaCaracter('Francicleudo');
// console.log(palavra);
...
*/
  ...
  const mostrar = value => {console.log(value)};

  const converteMinuscula = palavra => palavra.toLowerCase();

  const converter = converteMinuscula("CHOCOTONE E OVO DE PÁSCOA JUNTOS NO MERCADO EM PLENO FEVEREIRO");
...
*/
  ...
  // mostrar(getIndex('c','Francicleudo'));
...
*/
    const IsItemIncluided = (item, array) => array.includes(item);
    // mostrar(IsItemIncluided(2 , [3,2,7,9]));
...
*/
      ...
      const concatenar = (arrays1,arrays2) => arrays1.concat(arrays2);
      // mostrar(concatenar([15,36,36],[45,68,47]));
...
*/
  ...
  // mostrar(ultimoItem(['maca','pera','uva','abacaxi']));
...
*/
    const callCallback = (value, callback) => {
      ...
      // console.log(callCallback(33,triplo)); 




    const Resultriplo = (trip, multTres) => {
      return multTres(trip)
    }

      const multTres = (number = 0) => {
        let triplo = number * 3;
        mostrar(triplo)
      };
        
      // Resultriplo(55,multTres)
...
numbers.forEach((element,index,array) => {
  // mostrar( `"O ${index + 1}º item do array [${array.join(', ')}] é ${element}."`)
});
...
const getLikeMessage = (names = [])=>{
  switch (names.length) {
    ...
    case 3:
      return `${names[0]} , ${names[1]} e ${names[2]} isso!`;
  
    default:
      return  `${names[0]} , ${names[1]} e mais ${names.length -2} isso!`;;
  }
}

mostrar(getLikeMessage(['Fran','kele','Sara','Maria','Mateus']));