...
for (let j = 0; j < cities.length; j++) {
   upperCaseCities=[cities[j].toUpperCase()]
  //  console.log(upperCaseCities);
  }
  // console.log(upperCaseCities[0])
...
for(let k = 0;k < names.length;k++){
    HTMLTemplete += `<li>${names[k]}</li> \n`
  }

  // document.write(HTMLTemplete);
...
for (let l = 0; l < numbers.length; l++) {
  // soma += Number(numbers[l])
//  console.log(soma += Number(numbers[l]))
  // soma += numbers[l]
}
...
for (let y = 0; y < arrays.length; y++) {
  for (let a = 0; a < arrays[y].length; a++) {
   somaarray += arrays[y][a]
  }
}
